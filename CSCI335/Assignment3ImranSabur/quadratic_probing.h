//Imran Sabur
// Imran added locate(); in order to assist in probing the words 
...
class HashTable {
 public:
  ...
  explicit HashTable(size_t size = 101) : array_(NextPrime(size)),collision_counter(0)
    { MakeEmpty(); }
  ...
  */
  bool locate(const HashedObj& y){
    ...
    //Part Imran added
    if(array_[current_pos].info_ == EMPTY){ //condition if the element is not there
      return false; 
    }else if(array_[current_pos].element_ == y){
      return true;
    }
...
//function that returns the number of collisions
  int get_collisions(){
 ...
 private:        
  ...
  mutable int collision_counter = 0;
  mutable int probe_counter = 1;
