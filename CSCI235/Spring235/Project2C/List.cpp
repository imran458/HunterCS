...
// CSCI 235 Project 2B
//This program was modified by Imran Sabur for class CSCI 235. The scansublist method was added. 
...
List<T> List<T>::scanSublist(size_t position)
{
    ...
    Node<T>* pos_ptr = getPointerTo(position); //gets pointer to the specified position
    Node<T>* next_ptr = pos_ptr->getNext(); //next pointer which points to the next pointer of position pointer
    Node<T>* prev_ptr = pos_ptr->getPrevious(); //points to the previous pointer


    while(prev_ptr != nullptr && prev_ptr->getItem() <= pos_ptr->getItem()) //while the previous pointer is not null and previous item is
    {                                                                       //less than current item 
        pos_ptr = prev_ptr; // the current ptr be
        prev_ptr = prev_ptr->getPrevious(); //update the previous pointer to go back


    }
    
    next_ptr = pos_ptr->getNext(); //next pointer will point to 3 and not 6 in the list [3,4,5,6,3,4,5,9,5,4,3,2,5,7,4]]
    sublist.insert(sublist.getLength(),pos_ptr->getItem()); // inserts the element from the left which satify the condition

    while(next_ptr != nullptr && next_ptr->getItem() >= pos_ptr->getItem()) //now traverse right 
    {
        sublist.insert(sublist.getLength(),next_ptr->getItem()); //add all elements satisfying conditions
        pos_ptr = next_ptr; //current pointer becomes next 
        next_ptr = next_ptr->getNext(); //shift this pointer as well
    }
